...
async function start(update, context) {
  ...
  const keyboard = [
    [{ text: "馃摵 Fetch Channels", callback_data: "fetch_channels" }],
    [{ text: "馃搨 Upload IPTV File", callback_data: "upload_file" }],
    [
      { text: "鈩癸笍 About", callback_data: "about" },
      { text: "鉂� Help", callback_data: "help" }
    ]
  ];
  const reply_markup = { inline_keyboard: keyboard };

  await sendMessage(chat_id, "Welcome to **IPTV Bot**! 馃幀\n\nChoose an option below:", reply_markup);
}
...
async function broadcastMessage(broadcastText) {
  ...
  for (const chat_id of userContexts.keys()) {
    promises.push(sendMessage(chat_id, `馃摙 Broadcast:\n\n${broadcastText}`));
  }
...
async function button_handler(update, context) {
  ...
  if (query.data === "fetch_channels") {
    ...
    if (channels && channels.length > 0) {
      ...
      await sendMessage(chat_id, "鉂� No channels found or an error occurred.");
    }
    context.state = null;
    return;
  }
  ...
  // For "upload_file" button, prompt the user to upload an .m3u IPTV file.
    ...
    await sendMessage(chat_id, "馃搨 Please upload your .m3u IPTV file:");
    context.state = FILE_UPLOAD;
    return;
  }
  else if (query.data === "help") {
    await sendMessage(chat_id, "鉂� *Help Menu*\n\nThis bot allows you to:\n" +
      "馃摵 Fetch channels from a default IPTV URL\n" +
      "馃敆 \n" +
      "馃搨 Upload an .m3u IPTV file\n" +
      "馃幀 Click on a channel to get a direct streaming link.\n\n" +
      "Admin Commands:\n" +
    ...
    await sendMessage(chat_id, "鈩癸笍 *About*\n\nThis is an IPTV bot that helps you fetch and view IPTV channels.\n" +
      "Developed with 📡 by HelzTech❤️😉.");
    context.state = null;
    return;
  }
  context.state = null;
...
async function send_channel_page(obj, page_number, context) {
  ...
  if (start_index > 0) {
    pagination_buttons.push({ text: "鈫� Prev", callback_data: `page_${page_number - 1}` });
  }
  if (end_index < channels.length) {
    pagination_buttons.push({ text: "Next 鈫�", callback_data: `page_${page_number + 1}` });
  }
...
async function channel_selected(update, context) {
  ...
  if (index >= 0 && index < channels.length) {
    ...
    await sendMessage(query.message.chat.id, `馃幀 *${channel_name}*\n\n馃敆 Stream Link: \`${channel_url}\``);
  } else {
    await sendMessage(query.message.chat.id, "鉂� Channel not found.");
  }
}
...
async function handle_file_upload(update, context) {
  ...
  if (!fileResponse.ok) {
    await sendMessage(chat_id, "鉂� Failed to get the file.");
    context.state = null;
    return;
  }
  ...
  try {
    ...
    if (update.message) {
      ...
      // Command: /admin for admin panel
        ...
        if (chat_id !== ADMIN_ID) {
          await sendMessage(chat_id, "鉂� You are not authorized to access admin commands.");
        } else {
      ...
      // Command: /stats for admin to view bot stats
        ...
        if (chat_id !== ADMIN_ID) {
          await sendMessage(chat_id, "鉂� You are not authorized to view stats.");
        } else {
          const subscriberCount = userContexts.size;
          await sendMessage(chat_id, `馃搳 Stats:\n\nSubscribers: ${subscriberCount}`);
        }
      ...
      // Handle admin broadcast message when in ADMIN_BROADCAST state
        ...
        await sendMessage(chat_id, "鉁� Broadcast message sent to all subscribers.");
        context.state = null;
      }
