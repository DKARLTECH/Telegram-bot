...
/* Global admin set including the primary admin */
...
const ADMIN_INVITE = "ADMIN_INVITE";
...
async function adminPanel(update, context) {
  ...
  const adminKeyboard = [
    ...
    [{ text: "Invite Admin", callback_data: "admin_invite" }, { text: "View IPTV URL", callback_data: "admin_view_url" }]
  ];
...
async function button_handler(update, context) {
  ...
  // Handle admin inline commands if sent by an admin
  if (admins.has(chat_id)) {
    if (query.data === "admin_broadcast") {
      ...
      context.state = null;
      return;
    } else if (query.data === "admin_invite") {
      await sendMessage(chat_id, "📝 Please provide the Telegram ID of the admin to invite:");
      context.state = ADMIN_INVITE;
      return;
    } else if (query.data === "admin_view_url") {
      await sendMessage(chat_id, `🔗 Current IPTV URL: \n${DEFAULT_IPTV_URL}`);
      context.state = null;
      return;
    }
  }
  ...
  // For "upload_file" button, prompt the user to upload an .m3u IPTV file.
    ...
    await sendMessage(chat_id, "❓ *Help Menu*\n\nThis bot allows you to:\n" +
      ...
      "📂 Upload an .m3u IPTV file to get channels\n" +
...
async function send_channel_page(obj, page_number, context) {
  ...
  if (start_index > 0) {
    pagination_buttons.push({ text: "⬅️ Prev", callback_data: `page_${page_number - 1}` });
  }
  if (end_index < channels.length) {
    pagination_buttons.push({ text: "Next ➡️", callback_data: `page_${page_number + 1}` });
  }
...
/* Send a page of search results as buttons arranged similar to channel pages.
   This function is used for channel search results.
*/
async function send_search_results_page(obj, page_number, context) {
  const channels = context.user_data.searchResults || [];
  const start_index = page_number * CHANNELS_PER_PAGE;
  const end_index = start_index + CHANNELS_PER_PAGE;
  const page_channels = channels.slice(start_index, end_index);

  // Create buttons for each channel on this search result page with new callback_data prefix
  const keyboard = [];
  for (let i = 0; i < page_channels.length; i++) {
    const name = page_channels[i][0];
    keyboard.push([{ text: name, callback_data: `search_channel_${start_index + i}` }]);
  }

  // Add pagination buttons in a separate row
  const pagination_buttons = [];
  if (start_index > 0) {
    pagination_buttons.push({ text: "⬅️ Prev", callback_data: `search_page_${page_number - 1}` });
  }
  if (end_index < channels.length) {
    pagination_buttons.push({ text: "Next ➡️", callback_data: `search_page_${page_number + 1}` });
  }
  if (pagination_buttons.length > 0) {
    keyboard.push(pagination_buttons);
  }
  ...
  const reply_markup = { inline_keyboard: keyboard };
  const text_message = "🔍 *Search Results:*\n\nClick on a channel to get its stream link.";
  // If this is a callback query, edit the existing message to update the search result list.
  if (obj.message && obj.message.message_id) {
    await editMessageText(obj.message.chat.id, obj.message.message_id, text_message, reply_markup);
  } else {
    await sendMessage(obj.chat.id, text_message, reply_markup);
...
/* Handle page navigation (Next/Previous) for default channel list */
...
/* Handle search page navigation (Next/Previous) for search results */
async function search_page_navigation(update, context) {
  const query = update.callback_query;
  await telegramRequest("answerCallbackQuery", { callback_query_id: query.id });

  const page_number = parseInt(query.data.split("_")[2], 10);
  await send_search_results_page(query, page_number, context);
}
...
/* Handle search channel selection */
async function search_channel_selected(update, context) {
  const query = update.callback_query;
  await telegramRequest("answerCallbackQuery", { callback_query_id: query.id });

  const index = parseInt(query.data.split("_")[2], 10);
  const channels = context.user_data.searchResults || [];
...
async function handleRequest(request) {
  try {
    ...
    if (update.message) {
      ...
      else if (context.state === ADMIN_BROADCAST && admins.has(chat_id) && update.message.text) {
        await broadcastMessage(update.message.text);
        await sendMessage(chat_id, "✅ Broadcast message sent to all subscribers.");
        context.state = null;
      }
      // Handle admin invite when in ADMIN_INVITE state
      else if (context.state === ADMIN_INVITE && admins.has(chat_id) && update.message.text) {
        const newAdminId = parseInt(update.message.text, 10);
        if (!isNaN(newAdminId)) {
          admins.add(newAdminId);
          await sendMessage(chat_id, `✅ Admin with ID ${newAdminId} invited successfully.`);
        } else {
          await sendMessage(chat_id, "❗ Invalid Telegram ID provided.");
        }
        context.state = null;
      }
      ...
      // Handle channel search by entered text (e.g., "Sports")
      else if (update.message.text) {
        // Use the entered text as a search query if channels are available
        if (context.user_data.channels && context.user_data.channels.length > 0) {
          const searchQuery = update.message.text.trim().toLowerCase();
          const filteredChannels = context.user_data.channels.filter(channel => channel[0].toLowerCase().includes(searchQuery));
          if (filteredChannels.length > 0) {
            context.user_data.searchResults = filteredChannels;
            await send_search_results_page(update.message, 0, context);
          } else {
            await sendMessage(chat_id, "❗ No channels matching your search were found.");
          }
        } else {
          await sendMessage(chat_id, "❗ No channels available to search. Please fetch channels first.");
        }
      }
    } else if (update.callback_query) {
      const data = update.callback_query.data;
      if (data.startsWith("search_page_")) {
        await search_page_navigation(update, context);
      } else if (data.startsWith("search_channel_")) {
        await search_channel_selected(update, context);
...
// This completes the translation of the original code to JavaScript for deployment on Cloudflare Workers with the additional admin functionalities and channel search by text input.
