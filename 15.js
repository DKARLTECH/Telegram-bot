...
/* Admin function: Broadcast message to all subscribers */
async function broadcastMessage(text) {
  let count = 0;
  for (const [chat_id] of userContexts) {
...
/* Admin function: Get bot stats */
function getStats() {
  return {
    subscribers: userContexts.size,
  };
}
...
async function handleRequest(request) {
  try {
    ...
    if (update.message) {
      ...
      if (chat_id === OWNER_ID && update.message.text) {
        // Command: /broadcast <message>
        if (update.message.text.startsWith("/broadcast ")) {
          const broadcastText = update.message.text.substring(11).trim();
          if (broadcastText) {
            const count = await broadcastMessage(broadcastText);
            await sendMessage(chat_id, `✅ Broadcast sent to ${count} subscribers.`);
          } else {
            await sendMessage(chat_id, "❌ Please provide a message to broadcast. Usage: /broadcast Your message here");
          }
          return new Response("OK", { status: 200 });
        }
        // Command: /stats
        else if (update.message.text.startsWith("/stats")) {
          const stats = getStats();
          await sendMessage(chat_id, `📊 *Bot Stats:*\n\nSubscribers: ${stats.subscribers}`);
          return new Response("OK", { status: 200 });
        }
      }
      // Command: /start
