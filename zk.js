...
// Process uploaded .m3u file from Telegram
async function processUploadedM3UFile(message) {
  const chat_id = message.chat.id;
  const file_id = message.document.file_id;
  const fileUrl = `${TELEGRAM_API}/getFile?file_id=${file_id}`;
  try {
    ...
    await sendMessage(chat_id, "Error processing uploaded file.");
  }
}
...
async function listCustomChannels(chat_id, page, message_id = null) {
  ...
  const perPage = CHANNELS_PER_PAGE;
...
// Play a custom channel inside Telegram
