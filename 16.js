...
async function handle_url_input(update, context) {
  const chat_id = update.message.chat.id;
  // Trim the input to remove extraneous whitespace.
  const user_url = update.message.text.trim();
  // Validate that the URL starts with http:// or https://
  if (!/^https?:\/\//i.test(user_url)) {
    await sendMessage(chat_id, "❌ Invalid IPTV URL provided. Please ensure it starts with http:// or https://");
    context.state = null;
    return;
  }
