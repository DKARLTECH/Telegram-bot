...
async function start(update, context) {
  ...
  const keyboard = [
    ...
    [{ text: "🔗 Paste Your IPTV URL", callback_data: "paste_url" }],
...
async function button_handler(update, context) {
  ...
  // For "paste_url" button, prompt the user to paste their IPTV URL.
  else if (query.data === "paste_url") {
    await sendMessage(chat_id, "🔗 Please paste your IPTV URL:");
    context.state = URL_INPUT;
    return;
  }
...
async function handleRequest(request) {
  try {
    ...
    // Add chat id to subscribers list
    ...
    if (update.message && update.message.text) {
      ...
      // Command: /stats - view bot statistics (accessible by owner and admins)
      if (text.startsWith("/stats")) {
        if (update.message.from.id === OWNER_ID || admins.has(update.message.from.id)) {
      ...
      // Command: /broadcast - broadcast a message to all subscribers (accessible by owner and admins)
      if (text.startsWith("/broadcast")) {
        if (update.message.from.id === OWNER_ID || admins.has(update.message.from.id)) {
          let parts = text.split(" ");
          parts.shift(); // remove command
          let broadcastMsg = parts.join(" ");
          for (let subscriber of subscribers) {
            // Send broadcast message to each subscriber
            telegramRequest("sendMessage", { chat_id: subscriber, text: broadcastMsg });
          }
          await sendMessage(chat_id, "✅ Broadcast sent.");
        } else {
          await sendMessage(chat_id, "❌ Unauthorized.");
        }
        return new Response("OK", { status: 200 });
      }
      // Command: /addadmin - add a new admin (accessible only by the owner)
      if (text.startsWith("/addadmin")) {
        if (update.message.from.id === OWNER_ID) {
          let parts = text.split(" ");
          if (parts.length < 2) {
            await sendMessage(chat_id, "❌ Please provide a valid chat ID to add as admin.");
            return new Response("OK", { status: 200 });
          }
          let newAdminId = parseInt(parts[1], 10);
          if (isNaN(newAdminId)) {
            await sendMessage(chat_id, "❌ Invalid chat ID.");
            return new Response("OK", { status: 200 });
          }
          admins.add(newAdminId);
          await sendMessage(chat_id, `✅ Added admin with ID: ${newAdminId}`);
        } else {
          await sendMessage(chat_id, "❌ Unauthorized.");
        }
        return new Response("OK", { status: 200 });
      ...
      // Command: /start and handling other message types
      // Command: /start
      if (text.startsWith("/start")) {
        await start(update, context);
      }
      // Check for custom IPTV URL input
...
// This completes the translation of the original Python code to JavaScript for deployment on Cloudflare Workers.
