...
async function processTelegramMessage(message) {
  ...
  if (text === "/start") {
    // Automatically subscribe users on /start command and notify lifetime subscription immediately
    subscribers.add(chat_id);
    await sendMessage(chat_id, "You are now subscribed for lifetime notifications.");
    let keyboard = {
      inline_keyboard: [
...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    if (channels.length === 0) {
      await sendMessage(chat_id, "No channels found in the custom file.");
      return;
    }
    
    // Store the custom channels for this user in customChannelsMap
    customChannelsMap.set(chat_id, channels);
    // Display custom channels (first page)
...
async function processUploadedM3UFile(message) {
  const chat_id = message.chat.id;
  // Retrieve the file information from Telegram
  let fileUrl = `${TELEGRAM_API}/getFile?file_id=${message.document.file_id}`;
  try {
    const fileResponse = await fetch(fileUrl, { method: "GET" });
    const fileData = await fileResponse.json();
    if (!fileData.ok) {
      await sendMessage(chat_id, "Failed to get file info.");
      return;
    }
    const filePath = fileData.result.file_path;
    const downloadUrl = `https://api.telegram.org/file/bot${BOT_TOKEN}/${filePath}`;
    const fileContentResponse = await fetch(downloadUrl, { method: "GET" });
