...
async function handle_url_input(update, context) {
  ...
  // Validate that the pasted text is a valid URL format.
  if (!user_url.startsWith("http://") && !user_url.startsWith("https://")) {
    await sendMessage(chat_id, "❌ Invalid URL. Please ensure your URL starts with http:// or https://");
    context.state = null;
    return;
  }
