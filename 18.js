...
/* Helper function to send a document (file upload) */
async function sendDocument(chat_id, filename, fileContent, caption = "") {
...
async function button_handler(update, context) {
  ...
  // For "upload_file" button, prompt the user to upload an .m3u IPTV file.
    ...
    await sendMessage(chat_id, "❓ *Help Menu*\n\nThis bot allows you to:\n" +
      ...
      "🔗 Paste your own IPTV URL (which will be converted to a channels file)\n" +
...
/* Handle custom IPTV URL input (used for the "paste_url" option)
   Changed behavior: After fetching channels from the provided IPTV URL,
   convert the channels into an m3u file format and upload it to the user as "channels.m3u". */
async function handle_url_input(update, context) {
  ...
  if (channels && channels.length > 0) {
    // Build m3u file content from channels array
    let fileContent = "#EXTM3U\n";
    for (let i = 0; i < channels.length; i++) {
      fileContent += `#EXTINF:-1,${channels[i][0]}\n${channels[i][1]}\n`;
    }
    // Send the m3u file as a document
    await sendDocument(chat_id, "channels.m3u", fileContent, "Here is your converted IPTV file.");
  } else {
    await sendMessage(chat_id, "❌ No channels found or an error occurred.");
  }
  context.state = null;
