...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    if (channels.length === 0) {
      await sendMessage(chat_id, "No channels found in submitted custom file.");
      return;
    }
    
    // Store the custom channels for this user in the customChannelsMap
    customChannelsMap.set(chat_id, channels);
    await sendMessage(chat_id, "Custom channels updated successfully.");
    // Display the custom channels list so the user can select a channel to play
...
// List custom channels for the user with pagination support
async function listCustomChannels(chat_id, page, message_id = null) {
  let channels = customChannelsMap.get(chat_id) || [];
  if (channels.length === 0) {
    if (message_id) {
      await editMessage(chat_id, message_id, "No custom channels found. Please add a custom URL first.");
    } else {
      await sendMessage(chat_id, "No custom channels found. Please add a custom URL first.");
    }
    return;
  }
...
async function processUploadedM3UFile(message) {
  ...
  try {
    ...
    let fileResponse = await fetch(`${TELEGRAM_API}/getFile?file_id=${fileId}`);
    const fileData = await fileResponse.json();
    if (!fileData.ok) {
      await sendMessage(chat_id, "Failed to retrieve the uploaded file.");
      return;
    }
    ...
    if (!response.ok) {
      await sendMessage(chat_id, "Failed to download the uploaded file.");
      return;
    }
    const content = await response.text();
    ...
    if (channels.length === 0) {
      await sendMessage(chat_id, "No channels found in uploaded file.");
      return;
    }
    // Save the uploaded file channels as the custom channels for this user
    customChannelsMap.set(chat_id, channels);
    await sendMessage(chat_id, "Custom channels updated successfully from uploaded file.");
    await listCustomChannels(chat_id, 0);
  } catch (error) {
